    ...
    <div className="min-h-screen bg-[#06091b] text-white overflow-x-hidden selection:bg-indigo-500/30 relative flex flex-col justify-between">
      ...
      <main className="relative z-10 flex-grow pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full flex flex-col justify-center items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl px-4">
          ...
          {/* Contributor Leaderboard (LOCKED) */}
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.2 }}
            className="relative rounded-3xl overflow-hidden border border-white/5 bg-[#0c102b]/10 p-8 backdrop-blur-md flex flex-col justify-between group shadow-2xl min-h-[380px] cursor-not-allowed select-none"
          >
            {/* Glowing Accent Top Border */}
            <div className="absolute top-0 left-0 h-[2px] w-full bg-gradient-to-r from-transparent via-slate-500/30 to-transparent" />

            <div>
              {/* Top Row: Icon & Status */}
              <div className="flex items-center justify-between mb-8">
                <div className="w-14 h-14 rounded-2xl bg-slate-500/5 border border-white/5 flex items-center justify-center text-slate-500">
                  <Lock size={24} />
                </div>
                <span className="px-3 py-1 rounded-full text-[9px] font-extrabold font-mono uppercase tracking-wider border bg-white/5 text-slate-400 border-white/10">
                  LOCKED
                </span>
              </div>

              {/* Title & Desc */}
              <h2 className="text-xl sm:text-2xl font-bold text-slate-400 mb-3 tracking-tight">
                Contributor Leaderboard
              </h2>
              <p className="text-slate-500 text-xs sm:text-sm font-light leading-relaxed mb-6">
                Rankings based on successful pull requests, solved issues, and code contributions to ASOC cohort repositories.
              </p>
            </div>

            {/* Bottom State */}
            <div className="pt-6 border-t border-white/5 mt-auto flex items-center justify-between text-xs text-slate-600 font-mono">
